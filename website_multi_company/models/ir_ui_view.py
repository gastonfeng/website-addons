...
class IrUiView(models.Model):
    ...
    def make_multi_website(self):
        self.ensure_one()
        default_theme = self.env.ref('website_multi_theme.theme_default', raise_if_not_found=False)
        if not default_theme:
            raise exceptions.UserError(_('Oops, Something is wrong: Default theme is not found. Try to update module website_multi_theme'))
        ...
        if websites_without_theme:
            ...
            raise exceptions.UserError(_('Following Websites don\'t have Multi Theme: %s\n You need to update them first') % ', '.join(names))
        ...
        themes_without_default_theme = all_themes.filtered(
            lambda theme:
            default_theme not in theme.upstream_dependencies()
        )
        if themes_without_default_theme:
            names = themes_without_default_theme.mapped('name')
            raise exceptions.UserError(_('Following multi-themes don\'t have Default Theme as Sub-Theme: %s\n You need to update them first') % ', '.join(names))
